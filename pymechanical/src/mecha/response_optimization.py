...
class LaplacianOptimizer(PointMassOptimizer):
    """ Optimizes a network defined by a weighted Laplacian instead of
    a stiffness matrix
    """
    def num_optim_dof(self):
        return self.network.graph['E'].shape[1]

    def initialize(self, x0):
        """ Initialize the state for optimization of given initial conditions
        """
        self.Es = [self.network.incidence_matrix_ft(q).toarray() for q in self.qs]
        self.EHs = [E.conj().transpose() for E in self.Es]

        Ks = [np.eye(E.shape[0]) + np.dot(E*x0, EH) for E, EH in zip(self.Es, self.EHs)]
    ...
    def loss_and_jac_gap(self, x, ωsqrs, summed=True):
        ...
        values of ω^2.
        """
        Ks = [np.eye(E.shape[0]) + np.dot(E*x, EH) for E, EH in zip(self.Es, self.EHs)]
        ...
        grads = [2*np.sum(EH.transpose()*np.linalg.solve(G3, E), axis=0).real
                 for G, G3, E, EH in zip(Gs, G3s, self.Es, self.EHs)]
