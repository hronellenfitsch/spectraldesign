...
"""
  ...
  (c) Henrik Ronellenfitsch 2017-2020
"""
...
class MechanicalNetwork(nx.DiGraph):
    ...
    def edge_positions(self):
        ...
        into account.
        """
        x_a = np.array([self.nodes[a]['x'] for a, b in self.graph['edgelist']])
    ...
    def add_edge(self, *args, **kwargs):
        ...
        if compute and 'x' in self.nodes[u] and 'x' in self.nodes[v]:
            self.compute_b_hat(u, v)

        if compute_len and 'x' in self.nodes[u] and 'x' in self.nodes[v]:
            self.compute_len(u, v)
